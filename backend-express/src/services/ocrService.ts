...
class OCRService {
    ...
    private async preprocessImage(imagePath: string): Promise<string> {
        try {
            ...
            await image
                ...
                .resize(metadata.width && metadata.width > 2000 ? 2000 : undefined, metadata.height && metadata.height > 2000 ? 2000 : undefined, { fit: 'inside', withoutEnlargement: true })
    ...
    private parseBatteryData(text: string): {
        battery_code: string;
        brand?: string;
        voltage?: number;
        found: boolean;
    ...
    async extractFromFile(fileBuffer: Buffer, originalName?: string): Promise<OCRResult & {
        battery_code: string;
        brand?: string;
        voltage?: number;
    }> {
